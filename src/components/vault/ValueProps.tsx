...
export function ValueProps() {
  return (
    <section className="relative border-y border-border/40 bg-surface-1 py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-6">
        <Reveal className="mb-20 text-center">
          <p className="mb-4 text-[11px] uppercase tracking-[0.4em] text-gold">Il nostro standard</p>
          <h2 className="font-display text-4xl tracking-tight text-foreground md:text-5xl">
            Costruito per chi rifiuta <em className="text-gold italic">l'ordinario</em>.
          </h2>
        </Reveal>
        ...
        <div className="grid gap-px overflow-hidden rounded-sm border border-border/60 bg-border/60 md:grid-cols-3">
          {items.map((item, i) => (
            <Reveal key={item.title} delay={i * 0.15}>
              <div className="group h-full bg-surface-2 p-10 transition-colors duration-500 hover:bg-surface-1 md:p-12">
                ...
                <h3 className="font-display mb-4 text-2xl tracking-tight text-foreground">{item.title}</h3>
